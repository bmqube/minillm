...
impl GPTModel {
    ...
    pub fn forward(&self, token_ids: &[u32], weights: &ModelWeights) -> Tensor {
        let batch_size = 1; // For now, assume batch size of 1
        ...
        let wte_t = Tensor { data: wte.data.t().to_owned() };
    ...
    fn get_token_embeddings(&self, token_ids: &[u32], weights: &ModelWeights) -> Tensor {
        ...
        for &token_id in token_ids {
            ...
            assert!(token_id < vocab_size, "Token ID {} out of vocabulary range", token_id);
    ...
    fn get_position_embeddings(&self, seq_len: usize, weights: &ModelWeights) -> Tensor {
        ...
        assert!(seq_len <= max_pos, "Sequence length {} exceeds maximum position {}", seq_len, max_pos);
